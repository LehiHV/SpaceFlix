    ...
    switch($comando){
        ...
        case "Buy":
            ...
            $sqlCheck = "SELECT SP.Id,COUNT(*) as Cuentas_Validas
                        ...
                        WHERE C.Valido = true AND SP.Id = $id_producto
                        GROUP BY SP.Id";
            ...
            if ($result) {
                ...
                if ($row['Cuentas_Validas'] >= 1) {
                    ...
                    if ($creditos >= $pagar) {
                        ...
                        $sqlCuenta = "SELECT *
                                      FROM Cuentas
                                      WHERE Valido=TRUE AND Tipo_de_Cuenta=$tipo_de_cuenta
                                      LIMIT 1";
                                ...
                                // Crear Registro de Ticket
                                 ...
                                 $ticket_number = $conn->insert_id;
                                    if ($result->num_rows > 0) {
                                        ...
                                        $sqlCuentaSell = "SELECT * FROM Cuentas WHERE Id = $id_cuenta";
                                        $resultCuentaSell = $conn->query($sqlCuentaSell);
                                        if ($resultCuentaSell) {
                                            $cuenta_sell = $resultCuentaSell->fetch_assoc();
                                            echo json_encode(array("message" => "Compra realizada Exitosamente", "user" => $row, "account" => $cuenta_sell, "ticket"=>$ticketInfo,"prueba"=>$sqlRecarga));
                                        }
                                    }
                                    else {
